...
from array_numpy import input as aip
from array_numpy import cleaning as acl
from array_numpy import statistics as ast
from array_numpy import regression as areg
...
def input_data(input_list, input_dir):
    ...
    '''
    data_list = aip.read_data(input_list, input_dir)
    return data_list
...
def data_cleaning(data_list, user_dtstructure, user_name, file_names):
    ...
    # called by main.py
    # calling other modules in array_numpy 
    '''
    # get numbers of rows and columns
    nrow, ncol = acl.rows_and_columns(data_list)
...
that document these observations.' %{
    ...
    data_list = acl.drop_missing_values(data_list) # drop missings
    nrow, ncol = acl.rows_and_columns(data_list)   # update data shape after dropping
...
that document these observations.' %{
    ...
    data_list = acl.drop_outliers(data_list)     # drop outliers using IQR boundary
    nrow, ncol = acl.rows_and_columns(data_list) # update data shape after dropping
...
that document these observations.'  %{
    ...
    data_colnames = acl.compare_colnames(data_list, file_names) # get var names
    ...
    data_list = acl.align_colnames(data_list, data_colnames) # update col names
    data_colnames = acl.compare_colnames(data_list, file_names) # get new names
...
def summary_stats(df, user_name):
    ...
    df = ast.slice_data(df)
    df = ast.filter_asthma(df)
    ...
    ast.plot_asthma_by_type_year(df)
